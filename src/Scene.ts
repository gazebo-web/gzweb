...
export class Scene {
  ...
  private particleSystem: Nebula.System; 
...
Nebula.System.fromJSONAsync(emitterJson, THREE).then((system: Nebula.System) => {
  this.particleSystem = system;
  const spriteRenderer = new Nebula.SpriteRenderer(this.scene, THREE);
  this.particleSystem.addRenderer(spriteRenderer);
});
*/


    this.particleSystem = new Nebula.System();
    const particleEmitter = new Nebula.Emitter();
    const spriteRenderer = new Nebula.SpriteRenderer(this.scene, THREE);

    const boxZone = new Nebula.BoxZone(0, 75, 75);
    boxZone.friction = 0.0;

    // Set emitter rate (particles per second) as well as the particle initializers and behaviours
    particleEmitter
      .setRate(new Nebula.Rate(1000))
      .setPosition(new THREE.Vector3(0, 0, 0))
      .setRotation(new THREE.Vector3(0, 0, 0))
      .setInitializers([
        new Nebula.Position(boxZone),
        //new Nebula.Mass(0),
        
        // Width, height
        new Nebula.Radius(.1, .1),
        // min, max, center the average life value, isEnabled
        new Nebula.Life(10, 10, true, true),
        new Nebula.VectorVelocity(new Nebula.Vector3D(4, 0, 0), 0),
      ])
      .setBehaviours([
        //new Nebula.Alpha(1, 0),
        //new Nebula.Scale(.1, 1.3),
        new Nebula.Color(new THREE.Color(), new THREE.Color()),
      ]).emit();
  ...
  public render(): void {
    ...
    if (this.particleSystem) {
      ...
      this.particleSystem.update(clock.getDelta());
    }
